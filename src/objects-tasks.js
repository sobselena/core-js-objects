...
function shallowCopy(obj) {
  const clonePrototypeObj = Object.create(Object.getPrototypeOf(obj));
  return Object.assign(clonePrototypeObj, obj);
}
...
function isEmptyObject(obj) {
  const copyObj = { ...obj };
  return Object.keys(copyObj).length === 0;
}
...
function makeWord(lettersObject) {
  const len = Object.values(lettersObject).reduce((acc, arr) => {
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((obj) => {
    const arg = keySelector(obj);
    if (!result.has(arg)) {
      result.set(arg, []);
    }
    result.get(arg).push(valueSelector(obj));
  });
  return result;
}
